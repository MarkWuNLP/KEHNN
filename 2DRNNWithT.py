...
def predict_cnn(datasets,
    ...
    for filter_h in filter_hs:
        ...
        pool_sizes.append((img_h-filter_h+3, img_w-filter_w+1))
    ...
    cell = GRU_Cell(n_in=100,n_hidden=100,n_out=100)
    session_topic = T.mean(TWords[t],1)
    res_topic = T.mean(TWords[t2],1)
    W = theano.shared(value=ortho_weight(200),borrow=True)
    W4 = theano.shared(value=ortho_weight(200),borrow=True)
    W2 = theano.shared(value=ortho_weight(100),borrow=True)
    W3 = theano.shared(value=ortho_weight(100),borrow=True)
    ...
    sentence2vec = BiGRU(n_in=100,n_hidden=100,n_out=100,batch_size=batch_size)
    sentence2vec2 = BiGRU(n_in=100,n_hidden=100,n_out=100,batch_size=batch_size)


    q_emb = sentence2vec(llayer0_input,lxmask,return_list = True)
    r_emb = sentence2vec(rlayer0_input,rxmask,return_list = True)

    q_emb2 = sentence2vec2(cell1,lxmask,return_list = True)
    r_emb2 = sentence2vec2(cell2,rxmask,return_list = True)
    ...
    input4 = ReLU(T.batched_dot(q_emb,T.dot(r_emb2,W4).dimshuffle(0,2,1)))

    #input = T.concatenate([input0.dimshuffle(0,'x',1,2),input1.dimshuffle(0,'x',1,2),input2.dimshuffle(0,'x',1,2)],1)
    #input = T.concatenate([input2.dimshuffle(0,'x',1,2))],1)
    input = T.concatenate([input1.dimshuffle(0,'x',1,2),input2.dimshuffle(0,'x',1,2),
                           input3.dimshuffle(0,'x',1,2)],1)
    ...
    #print 'test micro f1',f1_score(labelys,pys,labels=[0,1,2],average='micro')
...
def train_cnn(datasets,
    ...
    #llayer0_input = _dropout_from_layer(rng,llayer0_input,0.5)
    #rlayer0_input = _dropout_from_layer(rng,rlayer0_input,0.5)
    ...
    test = theano.function([index], conv_layer.output, givens={
        lx: val_set_lx[index*batch_size:(index+1)*batch_size],
        lxmask:val_set_lx_mask[index*batch_size:(index+1)*batch_size],
        rx: val_set_rx[index*batch_size:(index+1)*batch_size],
        rxmask:val_set_rx_mask[index*batch_size:(index+1)*batch_size],
    ...
    #params +=[Words]
    params += [TWords]
    #grad_updates = opt.sgd_updates_adadelta(params, cost, lr_decay, 1e-6, sqr_norm_lim)
    ...
    for i in xrange(5):
        ...
        for minibatch_index in xrange(datasets[1].shape[0]/batch_size):
            ...
            for i in range(batch_size):
                ...
                labelys.append(labely[i])
            #print pys
        print "marco f1",f1_score(labelys,pys,labels=[0,1,2],average='macro')
        print "acc" , accuracy_score(labelys,pys)
...
def createtopicvec(word_idx_map):
    max_topicword = 20
    topicmatrix = np.zeros((200,max_topicword)).astype('int')
...
if __name__=="__main__":
    dataset = "preserve_stopc.bin"
    x = cPickle.load(open(dataset,"rb"))
    revs, wordvecs, max_l,tw = x[0], x[1], x[2], x[3]
    datasets = make_cnn_data(revs,wordvecs.word_idx_map,max_l=50)
    #tm = createtopicvec(wordvecs.word_idx_map)
    train_cnn(datasets,wordvecs.W,tw,filter_hs=[3],hidden_units=[8],batch_size=50)
    #predict_cnn(datasets,wordvecs.W,tw,filter_hs=[3],hidden_units=[8],batch_size=50)